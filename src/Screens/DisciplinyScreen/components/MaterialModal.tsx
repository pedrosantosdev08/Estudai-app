...
export default function MaterialModal({ visible, materia, onClose, onRequestDelete }: Props) {
  const { getConteudo, adicionarTexto, removerTexto, adicionarMidia, removerMidia } = useMateriais();
        ...
        <ScrollView contentContainerStyle={styles.modalContent}>
          ...
          {isAddingText && (
            <View style={styles.textInputWrapper}>
              ...
              <View style={styles.textActions}>
                <Button onPress={adicionarTextoLocal} disabled={textContent.trim() === ""}>Confirmar</Button>
          ...
          {conteudoAtual?.textos.map((t, i) => (
            <View key={i} style={styles.itemRow}>
              ...
              {isSelectingDelete && <Button onPress={() => removerTexto(materia.id, i)} compact>Remover</Button>}
            </View>
          ))}
          ...
          {conteudoAtual?.imagens.map((uri, i) => (
            <View key={i} style={styles.itemRow}>
              ...
              {isSelectingDelete && <Button onPress={() => removerMidia(materia.id, "imagens", i)} compact>Remover</Button>}
            </View>
          ))}
          ...
          {conteudoAtual?.videos.map((uri, i) => (
            <View key={i} style={styles.itemRow}>
              <Video source={{ uri }} style={styles.preview} useNativeControls resizeMode={ResizeMode.CONTAIN} isLooping />
              {isSelectingDelete && <Button onPress={() => removerMidia(materia.id, "videos", i)} compact>Remover</Button>}
            </View>
          ))}
          ...
          {conteudoAtual?.audios.map((uri, i) => (
            <View key={i} style={styles.itemRow}>
              <TouchableOpacity onPress={() => playAudio(uri, i)}>
                <Text style={{ marginTop: 8 }}>{playingAudioIndex === i ? "Reproduzindo..." : "Tocar Áudio"}</Text>
              </TouchableOpacity>
              {isSelectingDelete && <Button onPress={() => removerMidia(materia.id, "audios", i)} compact>Remover</Button>}
            </View>
          ))}

          {isSelectingDelete && <Button onPress={() => setIsSelectingDelete(false)} style={{ marginTop: 8 }}>Concluído</Button>}
        </ScrollView>
